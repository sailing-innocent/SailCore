...
Compiler::Compiler() {
	...
	m_bin_op_precedence['='] = 1; // assignment operator
	m_bin_op_precedence['<'] = 10;// less than operator
	m_bin_op_precedence['+'] = 20;// plus operator
	m_bin_op_precedence['-'] = 30;// minus operator
	m_bin_op_precedence['*'] = 40;// multiply operator
}
...
void Compiler::handle_for_expr() noexcept {
	auto for_ast = parse_for_expr();
	exprs.push_back(std::move(for_ast));
}

void Compiler::handle_simple_expr() noexcept {
...
// for loop expression
// for i = 1, 10, 1 ( <body> )
std::unique_ptr<ExprAST> Compiler::parse_for_expr() noexcept {
	get_next_tok();// eat 'for'
	if (m_cur_tok != static_cast<int>(Token::TOK_IDENTIFIER)) {
		// error; skip
		return nullptr;
	}
	std::string var_name = m_identifier_str;
	get_next_tok();// consume identifier
	...
	return std::make_unique<ForExprAST>(
		var_name, std::move(start), std::move(end), std::move(step), std::move(body));
}
